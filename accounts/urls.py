...
company space name will be suggested. this is optional, they can choose any kind of space name if available
username will be suggested. this is optional, they can choose any kind of username if available
first user will create the profile in our account in a company. He will have to invite the remaining people by allowing them to spaces. He can make invited users as root users as well.
...
urlpatterns = [
    ...
    path("verify_company_details_before_registering/", views.CustomerCompanyVerificationView.as_view(), name="verify_company_details_before_registering"), # it will verify the company name or space name/sub domain name already exists or not.
    ...
    # path("suggest_company_space/", views.SuggestCompanySpaceView.as_view(), name="suggest_company_space") # this will suggest a new company space name / sub domain name which is not already existed in the database.
]
